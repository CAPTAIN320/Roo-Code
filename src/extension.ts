...
/**
 * Check if we should auto-open the Code Captain sidebar after switching to a worktree.
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(
				`[Worktree] Auto-opening Code Captain sidebar for worktree: ${worktreeAutoOpenPath}`,
			)

			// Open the Code Captain sidebar with a slight delay to ensure UI is ready
			setTimeout(async () => {
				try {
					await vscode.commands.executeCommand("code-captain.plusButtonClicked")
				} catch (error) {
					outputChannel.appendLine(
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Initialize the provider *before* the Code Captain Cloud service.
	const provider = new ClineProvider(context, outputChannel, "sidebar", contextProxy, mdmService)

	// Initialize Code Captain Cloud service.
